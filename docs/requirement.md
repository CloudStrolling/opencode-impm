...
# 全局唯一需求编号规则
URS 的功能需求（FR）、非功能需求（NFR），以及 PRD 的功能编号（F）、用户故事编号（US）在**整个项目所有版本间全局唯一**，不允许仅在本版本内编号。编号格式统一为「前缀-v版本号-序号」，例如：FR-v0.0.1-001、NFR-v0.0.1-001、F-v0.0.1-001、US-v0.0.1-001。
- 序号在所属版本内从 001 顺序递增；
- 跨版本保持不变的需求 / 功能 / 用户故事沿用其历史编号，不重新编号；仅新增或变更的需求才启用新版本内新编号；
- 其他编号（TASK-xxx、TC-xxx、RTM-xxx）仍按版本内唯一即可，不受本规则影响。
- 后续如出现扩展功能需求（EFR）等新类别，一律套用同一格式「EFR-v版本号-序号」。
...
- 处理内容：
  ...
  4、不要将完整内容复制到docs/{项目英文缩写}-urs.md，而是从当前版本URS中提取摘要（需求编号/名称/描述摘要/优先级、业务目标/场景/约束/假设概要，含版本演进表与完整文档索引）写入docs/{项目英文缩写}-urs.md。摘要正文**不按版本号分节**，按「项目 → 模块 → 业务场景」重组并按业务逻辑排序（主流程在前、支线在后），版本信息由来源版本列与版本演进表承载。完整内容保留在版本目录文档中。
  5、URS中的功能需求（FR）、非功能需求（NFR）编号项目全局唯一，格式为「前缀-v版本号-序号」，如FR-v0.0.1-001。
...
- 处理内容：
  ...
  4、不要将完整内容复制到docs/{项目英文缩写}-prd.md，而是从当前版本PRD中提取摘要（功能清单F编号/名称/模块/优先级/版本范围、用户故事US编号/标题/摘要/优先级、产品背景/目标用户/数据需求/验收标准概要，含版本演进表与完整文档索引）写入docs/{项目英文缩写}-prd.md。摘要正文**不按版本号分节**，功能清单按「项目 → 模块」分组、模块内按业务逻辑排序，用户故事按所属功能/模块分组并逻辑排序，版本信息由来源版本/版本范围列与版本演进表承载。完整内容保留在版本目录文档中。
  5、PRD中的功能编号（F）、用户故事编号（US）项目全局唯一，格式为「前缀-v版本号-序号」，如F-v0.0.1-001、US-v0.0.1-001。
...
- agent：BA
 - 处理内容：
  ...
  3、需求编号采用项目全局唯一格式「前缀-v{当前版本号}-序号」，如FR-v{当前版本号}-001、NFR-v{当前版本号}-001；跨版本延续的已有需求沿用历史编号。
  4、将用户需求说明书存放到版本文件的目录：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-urs-v{当前版本号}.md。
  5、版本进度文件：docs/{项目英文缩写}-v{当前版本号}/version_progress.md。增加第一行：步骤序号：前一序号+1，步骤名称：impm-urs-create，步骤状态：已完成。
...
- agent：BA
 - 处理内容：
  1、从模板目录下读取PRD-TEMPLATE.MD模板文件
  2、根据用户的输入，以及用户输入中提到的文件，按照模板文件的格式生成用户需求说明书。
  3、功能编号（F）、用户故事编号（US）采用项目全局唯一格式「前缀-v{当前版本号}-序号」，如F-v{当前版本号}-001、US-v{当前版本号}-001；跨版本延续的已有功能/用户故事沿用历史编号。
  4、将用户需求说明书存放到版本文件的目录：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-prd-v{当前版本号}.md。
    5、版本进度文件：docs/{项目英文缩写}-v{当前版本号}/version_progress.md。增加第一行：步骤序号：前一序号+1，步骤名称：impm-prd-create，步骤状态：已完成。
...
3. 根据当前任务的测试用例，找到对应的测试函数和测试脚本，并执行测试脚本。
...
- agent：TW
- 处理内容：合并采用「重构式合并」，按项目组织方式、代码结构和系统架构组织主文档结构，而非简单追加。
  1. URS：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-urs-v{当前版本号}.md 合并到docs/{项目英文缩写}-urs.md，只合并**摘要**（需求编号FR/NFR、名称、一行描述、优先级、业务归属），**不按版本号分节**，按「项目 → 模块 → 业务场景」重组并按业务逻辑排序（主流程在前、支线在后），版本信息由来源版本列与版本演进表承载；不合并完整内容；主文档不存在就先创建。完整内容保留在版本目录文档中。
  2. PRD：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-prd-v{当前版本号}.md 合并到docs/{项目英文缩写}-prd.md，只合并**摘要**（功能清单F、用户故事US），**不按版本号分节**，功能清单按「项目 → 模块」分组、模块内按业务逻辑排序，用户故事按所属功能/模块分组并逻辑排序，版本信息由来源版本/版本范围列与版本演进表承载；不合并完整内容；主文档不存在就先创建。
  3. API：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-api-v{当前版本号}.md 合并到docs/{项目英文缩写}-api.md，按**模块分组重构式合并**：新接口插入所属模块组，已有接口原位更新并标注版本，废弃接口移入下线小节。
  4. DBD：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-dbd-v{当前版本号}.md 合并到docs/{项目英文缩写}-dbd.md，按业务域/模块→表**重构式合并**：新表插入所属业务域，已有表原位更新并标注变更版本，废弃表移入废弃表小节，ER图同步更新。
  5. SQL：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-dbd-v{当前版本号}.sql 合并到docs/{项目英文缩写}-dbd.sql，**重构式合并**：新表追加CREATE、既有表生成ALTER或更新原CREATE，保持可从头完整执行、无重复冲突，每段标注来源版本。
  6. LLD：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-lld-v{当前版本号}.md 合并到docs/{项目英文缩写}-lld.md，按模块/业务流程**重构式合并**：新模块插入与架构一致的位置，已有模块原位更新并标注修订版本。
  7. 合并时引用结构基准（docs/project.md 项目地图、docs/{项目英文缩写}-sad.md 系统架构），主文档文档头维护版本演进表，保证版本可追溯。
  8. 版本进度文件：docs/{项目英文缩写}-v{当前版本号}/version_progress.md。增加第一行：步骤序号：前一序号+1，步骤名称：impm-doc-merge，步骤状态：已完成。
...
- 在标准 impm-init / impm-docs / impm 流程基础上，提供"文档审核"版：
...
3.  /impm-init-review 执行项目初始化阶段的所有步骤（且 project/urs/prd/sad/dbd/api/lld/task/testcase 逐文档用户审核）。
4.  /impm-docs 执行需求分析整理阶段的所有步骤。
5.  /impm-coding 执行需求分析整理阶段的所有步骤。
6.  /impm-finish 执行回归测试和版本文档整理阶段的所有步骤。
7.  /impm-docs-review 执行需求分析整理阶段的所有步骤（且 urs/prd/sad/dbd/api/lld/task 逐文档用户审核）。
