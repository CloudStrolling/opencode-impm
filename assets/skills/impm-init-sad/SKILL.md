...
Call impm_template_reader(projectRoot, SAD-TEMPLATE.MD) to read the System Architecture Design document template, and clarify the template sections: design goals and constraints, technology stack selection, system context diagram, container diagram, component diagram, deployment architecture, security architecture, performance architecture, data flow diagram, architecture decision records.

### Step 2: reverse-engineer the architecture design
Read the existing documents via impm_doc_reader (focus on the PRD: docs/{project English abbreviation}-prd.md, as well as project, etc.), combine the current project code and documents, and fill in the SAD according to the template format:
- Existing project: reverse-engineer the architecture design sections from the existing code, technology stack, and documents.
...
Call impm_doc_writer(projectRoot, sad, {project Chinese name}, {current version number}, {task number}, main, content) to write the master document docs/sad.md (sad only has the master document, no in-version document; create it if the master document does not exist). Verify that docs/sad.md has been created and the content is complete.

### Step 4: record progress
Call impm_progress(projectRoot, {project English abbreviation}, {current version number}, add, impm-init-sad, completed) to record the completion of this step.

## Deliverables
- docs/sad.md

## Completion tips
- To continue to the next step, enter /impm-init-dbd
