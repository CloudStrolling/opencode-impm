...
description: Queries official documentation and usage of third-party packages, middleware, and SDKs, and writes the results to ws.md in the task directory to provide the reference basis for the coding development phase.
---
...
## When to Use
Use this skill during the coding development phase when the official documentation, usage, and examples of third-party middleware, packages, or SDKs used by the task need to be queried, or when web resources related to the task need to be searched.

## Executing Agent
This skill is executed by the WS subagent (subagent_type=ws). Load this skill with the Skill tool when executing.

## Dispatch Instructions (MUST be followed when the PM/upper-level orchestrator launches this skill)
1. Launch method: use the task tool to launch a subagent; subagent_type MUST be `ws`; the PM or orchestrator must not execute this skill's content on its own.
2. The task prompt MUST carry the context (indispensable): the absolute path of the project root (projectRoot), the project English abbreviation ({project abbreviation}), the current version ({current version}), the original user input $ARGUMENTS (including the file paths mentioned by the user), and the skill name (impm-task-coding-ws; require the subagent to load this skill with the Skill tool first before executing), and the task ID (taskId, extracted from $ARGUMENTS; when missing, use impm_task_manager to query the next executable task).
...
### Step 3: Identify the required third-party components
Based on the content of the files above, determine which third-party middleware, packages, or SDKs the current task requires.

### Step 4: Query official documentation and examples
Search the web for the official documentation, usage, and examples of these packages, including installation methods, core APIs, and common usage.

### Step 5: Verify version compatibility
When querying and collecting, check whether the version numbers used by the current project are compatible with the version numbers in the queried materials; if there are version differences, record them and provide recommendations.

### Step 6: Query materials related to the task
Also search and collect web resources related to the current task (e.g., algorithms, business solutions, troubleshooting experience, etc.).

### Step 7: Write ws.md
After analyzing, merging, and summarizing the queried content, call impm_doc_writer (docType=ws, taskId={task ID}) to write it to docs/{project abbreviation}-v{current version}/task_{task ID}/ws.md.

### Step 8: Record completion
Verify that ws.md has been generated with complete content, then call impm_progress (action=add, stepName=impm-task-coding-ws, status={task ID}-completed).
