...
description: The "document review edition" of the impm master workflow, which automatically executes all four phases of waterfall development (project initialization (with per-document user review), requirements analysis (with per-document user review), coding development, regression testing and version documentation), in which the project initialization phase uses impm-init-review (after each of the project/urs/prd/sad/dbd/api/lld/task/testcase steps completes, first display a document summary then pop up a prompt box to ask the user to review the document), and the requirements analysis phase uses impm-docs-review (after each of the urs/prd/sad/dbd/api/lld/task steps completes, first display a document summary then pop up a prompt box to ask the user to review the document).
---
...
## When to use
Use when the user requests executing the impm software engineering full workflow development and wants each initialization document and each document (URS, PRD, SAD, DBD, API, LLD, task list) to be reviewed and confirmed one by one by the user after it is generated during the project initialization and requirements analysis phases. This skill is fully consistent with the impm skill workflow; the only difference is that phase 1 uses impm-init-review instead of impm-init (per-document review for initialization documents), and phase 2 uses impm-docs-review instead of impm-docs.

## Execution role
This skill is executed (orchestrated) by the Project Manager (main Agent), who loads this skill with the Skill tool. Internal sub-steps must be dispatched to the corresponding subagents for execution according to the "General Dispatching Requirements" below; the PM only dispatches, checks, and makes decisions; **the user document review confirmation rounds (including summary extraction and display) in both phase 1 and phase 2 are executed directly by the PM and do not dispatch subagents**.

## General dispatching requirements (all sub-steps of this skill must comply)
1. Launch method: start the corresponding subagent for each sub-step with the task tool (subagent_type must match the mapping table below exactly) to execute the corresponding skill; the PM is prohibited from executing concrete work on behalf of the subagent (the only exception: the phase 1 and phase 2 user document review confirmation rounds are executed directly by the PM).
...
|----|----|----|
| Phase 1 | impm-init-review | PM (orchestrates, dispatches internally, with per-document user review) |
...
5. **The document review in both phase 1 (impm-init-review) and phase 2 (impm-docs-review) must first extract and display a document summary in the conversation dialog after each document is generated, then pop up a prompt box and wait for the user's explicit confirmation (review approved) before proceeding to the next step**; when the review is not approved, regenerate per the user's feedback and review again, never skip it.
6. Use English throughout.

## Execution steps

### Phase 1: Project initialization (impm-init-review, with per-document user review)
1. Load with the Skill tool and run the impm-init-review skill.
2. First run impm-init-isinit to determine whether the project has been initialized:
   ...
   - If it is an empty project or an existing project, execute all the initialization steps in sequence: impm-init-isinit → impm-init-git → impm-init-project (review) → impm-init-version → impm-init-urs (review) → impm-init-prd (review) → impm-init-sad (review) → impm-init-dbd (review) → impm-init-api (review) → impm-init-lld (review) → impm-init-task (review) → impm-init-testcase (review) → impm-init-commit. Steps marked "(review)" each have the PM first extract and display the document summary in the conversation dialog, then pop up a prompt box for the user to review the document, proceeding to the next step only after the review is approved.
...
2. Load with the Skill tool and run the impm-docs-review skill, executing in sequence: impm-version-create → impm-urs-create (review) → impm-prd-create (review) → impm-sad-update (review) → impm-dbd-create (review) → impm-api-create (review) → impm-lld-create (review) → impm-task-create (review) → impm-rtm-create → impm-analysis-commit. After each step marked "(review)" completes, the PM first extracts and displays the document summary in the conversation dialog, then pops up a prompt box for the user to review the document, proceeding to the next step only after the review is approved.
...
3. Report the complete deliverables of this version's development to the user, and explain the document review results (the review confirmation status of initialization documents in phase 1 and each document in phase 2).
...
- User review confirmation records of initialization documents in phase 1 and each document in phase 2 (completed in conversation, summaries not persisted to disk)
