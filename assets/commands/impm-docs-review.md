...
4. **After each urs/prd/sad/dbd/api/lld/task document is generated and verified correct, first read that document, extract a concise summary, and display it as text in the conversation (display only, do not write it to any file); then use the question tool to pop up a prompt box asking the user to review the document**; when the user selects "Approved", proceed to the next step; when "Needs Changes" is selected, re-dispatch the corresponding subagent per the user's feedback to regenerate and then display the summary and prompt the review again; during this period, do not advance to the next step.
